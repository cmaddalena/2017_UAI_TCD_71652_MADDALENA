    ...
    Private Sub btnInstalar_Click(sender As Object, e As EventArgs) Handles btnInstalar.Click
        If Not String.IsNullOrWhiteSpace(cmbServidores.Text) AndAlso Not String.IsNullOrEmpty(TextBox1.Text) Then
            If (CrearBase(cmbServidores.Text, TextBox1.Text)) Then
                MessageBox.Show(Module1.TRADUCIR_MSJ(6), "Mensaje de Sistema", MessageBoxButtons.OK, MessageBoxIcon.Information)
                CyrsaFC_Menu.Show()

                'Creacion_BBDD.Close()

            Else
                MessageBox.Show(Module1.TRADUCIR_MSJ(7), "Mensaje de Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error)
            End If
        Else
            MessageBox.Show(Module1.TRADUCIR_MSJ(7), "Mensaje de Sistema", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End If
